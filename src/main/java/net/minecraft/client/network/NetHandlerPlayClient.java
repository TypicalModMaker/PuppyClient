...
import dev.isnow.puppy.hook.MainMenuHook;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    public boolean onlineExecuted = false;
    ...
    public void onDisconnect(IChatComponent reason)
    {
        ...
        {
            this.gameController.displayGuiScreen(new GuiDisconnected(new GuiMultiplayer(new MainMenuHook()), "disconnect.lost", reason));
        }
    }
    ...
    {
        if(packetIn.getX() > 2000 || packetIn.getY() > 2000 || packetIn.getZ() > 2000 || packetIn.func_149144_d() > 2000 || packetIn.func_149147_e() > 2000 || packetIn.func_149149_c() > 2000 || packetIn.getStrength() > 2000 || packetIn.getAffectedBlockPositions() == null) {
            ChatHelper.printMessage("Server sent an invalid explosion packet, tried to crash the client");
            return;
        }
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
    ...
    public void handleTabComplete(S3APacketTabComplete packetIn)
    {
        ...
        if(fetchExecuted) {
            SaveLoad save = new SaveLoad("players.txt");
            String cocked = Arrays.toString(astring);
            save.item = cocked.substring( 1, cocked.length() - 1);

            save.save();
            fetchExecuted = false;
            ChatHelper.printMessage("Fetched " + astring.length + " players");
        }
        if(pluginsBypassExecuted) {
            if(astring.length != 0) {
                ChatHelper.printMessage("Plugins: &7" + Arrays.toString(Arrays.stream(astring).map(s -> s.replace(" ", ", ")).toArray(String[]::new)));
            }
            pluginsBypassExecuted = false;
        }

        if(pluginsTabExecuted) {
            ...
            while (i < length) {
                ...
                if (command.length > 1 && !plugins.contains(pluginName = command[0].replace("/", ""))) {
                    plugins.add(pluginName);
                }
                ++i;
            }
            Collections.sort(plugins);
            ChatHelper.printMessage("Plugins: &7" + plugins);
            pluginsTabExecuted = false;
        }
    }
    ...
    public boolean validateResourcePackUrl(NetHandlerPlayClient client, String url, String hash) {
        try {
            URI uri = new URI(url);
            ...
            if (!"http".equals(scheme) && !"https".equals(scheme) && !isLevelProtocol) {
                ...
                ChatHelper.printMessage("Resource pack URL is invalid or malformed " + url);
            }
            ...
            if (isLevelProtocol && (url.contains("..") || !url.endsWith("/resources.zip"))) {
                ...
                if (player != null) {
                    ChatHelper.printMessage("Server tried to exploit resourcepack packet with directory " + url);
                }
    ...
    public void handleResourcePack(S48PacketResourcePackSend packetIn)
    {
        ...
        this.netManager.sendPacket(new C19PacketResourcePackStatus(s1, C19PacketResourcePackStatus.Action.SUCCESSFULLY_LOADED));
