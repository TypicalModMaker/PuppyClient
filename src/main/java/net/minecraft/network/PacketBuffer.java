...
public class PacketBuffer extends ByteBuf {
    ...
    /**
     * Calculates the number of bytes required to fit the supplied int (0-5) if it were to be
     * read/written using readVarIntFromBuffer or writeVarIntToBuffer
     */
    ...
    /**
     * Reads a compressed int from the buffer. To do so it maximally reads 5 byte-sized chunks whose
     * most significant bit dictates whether another byte should be read.
     */
    ...
    /**
     * Writes a compressed int to the buffer. The smallest number of bytes to fit the passed int
     * will be written. Of each such byte only 7 bits will be used to describe the actual value
     * since its most significant bit dictates whether the next byte is part of that same int.
    ...
    public NBTTagCompound readNBTTagCompoundFromBuffer() throws IOException {
        ...
        if (b0 == 0) {
            ...
            this.readerIndex(i);
            return CompressedStreamTools.read(new ByteBufInputStream(this),
    ...
    /**
     * Reads a string from this buffer. Expected parameter is maximum allowed string length. Will
     * throw IOException if string length exceeds this value!
     */
    public String readStringFromBuffer(int maxLength) {
        ...
        if (i > maxLength * 4) {
            ...
            if (s.length() > maxLength) {
                throw new DecoderException(
                        "The received string length is longer than maximum allowed (" + i + " > "
                                + maxLength + ")");
            } else {
                return s;
            }
    ...
    public ByteBuf getBytes(
            int p_getBytes_1_, ByteBuf p_getBytes_2_, int p_getBytes_3_,
            int p_getBytes_4_
    ) {
    ...
    public ByteBuf getBytes(
            int p_getBytes_1_, byte[] p_getBytes_2_, int p_getBytes_3_,
            int p_getBytes_4_
    ) {
        return this.buf.getBytes(p_getBytes_1_, p_getBytes_2_, p_getBytes_3_, p_getBytes_4_);
    }
    ...
    public ByteBuf setBytes(
            int p_setBytes_1_, ByteBuf p_setBytes_2_, int p_setBytes_3_,
            int p_setBytes_4_
    ) {
    ...
    public ByteBuf setBytes(
            int p_setBytes_1_, byte[] p_setBytes_2_, int p_setBytes_3_,
            int p_setBytes_4_
    ) {
        return this.buf.setBytes(p_setBytes_1_, p_setBytes_2_, p_setBytes_3_, p_setBytes_4_);
    }
