...
public class S3FPacketCustomPayload implements Packet<INetHandlerPlayClient>
{
    ...
    public void processPacket(INetHandlerPlayClient handler)
    {
        ...
        if(Puppy.INSTANCE.dev) {
            try {
                if(channel.equals("REGISTER")) {
                    for(String splitted : getBufferData().toString(Charset.defaultCharset()).split("\0")) {
                        ChatHelper.printMessage("CHANNEL: " + splitted);
                    }
                } else {
                    try {
                        ChatHelper.printMessage("CHANNEL: " + this.channel + " DATA: " + getBufferData().toString(Charset.defaultCharset()));
                    } catch (Exception ignored) {
                        ChatHelper.printMessage("CHANNEL: " + this.channel);
                    }
                }
            } catch (Exception ignored) {
                ChatHelper.printMessage("Failed to read channels " + ignored.getMessage());
            }
        }
    }
